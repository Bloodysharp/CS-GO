...
DWORD WINAPI OnDLLAttach(LPVOID lpParameter) 
{
	...
	char temp[256];
	if (data.get_registry((char*)"Software\\fujiwara", (char*)"username", temp))
		data.get_registry((char*)"Software\\fujiwara", (char*)"username", login);
	if (data.get_registry((char*)"Software\\fujiwara", (char*)"password", temp))
		data.get_registry((char*)"Software\\fujiwara", (char*)"password", password);

	while (!GetModuleHandleA("serverbrowser.dll"))
		Sleep(200);

	console::initialize("fujiwarahack");
	console::log("fujiwarahack\n");
